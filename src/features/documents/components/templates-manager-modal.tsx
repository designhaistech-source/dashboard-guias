...
export function TemplatesManagerModal({ open, onOpenChange, kind }: TemplatesManagerModalProps) {
  ...
  const pending = confirmingDelete
    ? templates.find((t) => t.value === confirmingDelete) ?? null
    : null;
    ...
    return (
      <AppModal
        ...
        footer={
          <>
            ...
            <Button variant="destructive" autoFocus onClick={() => handleDelete(pending)}>
      ...
      >
        <div className="rounded-xl border border-border bg-background p-4">
          ...
          <p className="mt-1 text-sm font-medium text-foreground">{pending.label}</p>
        </div>
      </AppModal>
    );
  }
